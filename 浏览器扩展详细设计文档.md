...
- WebSocket 实时推送（持仓更新、订单更新、成交通知、状态心跳）。
...
│                       │  WS    │  /api/*  /ws/stream  │        │  strategy / config   │
│  - 状态区             │ ◄─────│                      │ ◄──────│  market_setup        │
│  - 下单区             │        │  - 读: snapshot()    │        │                      │
│  - 持仓区             │        │  - 写: 下单/撤单/平仓 │        │  ┌────────────────┐  │
│  - 全局指令           │        │  - 推: WS 事件       │        │  │  Polymarket    │  │
└───────────┬───────────┘        └──────────┬───────────┘        │  │  CLOB / Gamma  │  │
            │                               │                    │  │  / data-api    │  │
...
**推送流（FastAPI → 扩展，WebSocket）**：
- `position_update` — 某策略 `snapshot()` 变化（状态切换、持仓变化、cycle 递增）
- `order_update` — 挂单状态变化（新挂/成交/撤销）
- `fill_notify` — 成交通知（买单成交/卖单成交）
- `status_tick` — 周期性聚合状态心跳（对齐 `STATUS_EVERY_CYCLES`）
...
| GET | `/api/url/parse` | `?url=<polymarket_url>` | `{slug, event_title, markets: [MarketInfo]}` | 解析 URL |
...
连接后每秒推送 `status_tick`，状态变化时推送对应事件。
...
| event | 触发时机 | data |
|---|---|---|
| `position_update` | 策略 snapshot 变化（状态/持仓/cycle） | `StrategySnapshot` |
| `order_update` | 挂单状态变化（新挂/撤销） | `OrderInfo` |
| `fill_notify` | 成交（买单/卖单） | `{side, token_id, size, price, order_id}` |
| `status_tick` | 周期性聚合（对齐 `STATUS_EVERY_CYCLES`） | `StatusAggregate` |
| `error` | bot 异常 | `{code, message}` |
...
├── background.js           # service worker：tab 监听 + WS 连接管理
├── popup.html              # 弹窗：配置 FastAPI 地址
├── popup.js
├── api.js                  # FastAPI 客户端封装
├── ws.js                   # WebSocket 客户端（断线重连）
└── components/
    ├── StatusArea.js
...
- 监听 `chrome.tabs.onUpdated`，URL 变化时调用 `GET /api/url/parse`
...
sequenceDiagram
    ...
    WS-->>SP: position_update (cycle+1)

    Note over Bot: 等待成交...
    Bot->>Bot: 检测到买单成交
    Bot->>Bot: 立即挂保护卖单 (exit_position)
    WS-->>SP: fill_notify (BUY 成交)
    WS-->>SP: position_update (state=持仓中·已挂保护卖单)
    SP->>SP: 卡片状态切换 + 闪烁
    ...
    WS-->>SP: position_update (state=已平仓)
    SP->>SP: 卡片灰化
```
...
stateDiagram-v2
    [*] --> Connecting
    Connecting --> Online: onopen
    Online --> Reconnecting: onclose/onerror
    Reconnecting --> Online: 重连成功
    Reconnecting --> Offline: 重试超过 5 次
    Offline --> Connecting: 用户手动重连
    Online --> Online: 心跳每 30s
```

- 重连间隔：1s / 2s / 4s / 8s / 16s（指数退避，最多 5 次）
- 离线时顶部显示红色徽章"已断开 · [重连]"
- 重连成功后立即 `GET /api/positions` 全量同步
...
- FastAPI 不可达：顶部红色徽章"已断开"，UI 进入只读模式（仍可查看上次缓存的持仓，但所有写按钮禁用）
- WS 断线：状态区连接圆点变灰，自动重连，重连成功后全量同步
...
| 阶段 6 | WebSocket 实时推送 | position_update / fill_notify / status_tick |
